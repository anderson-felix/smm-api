...
const typeORMFilters: Record<string, any> = {
  ...
  /* NULL operations */
  null: (value: string) =>
    value.toLocaleLowerCase() === 'true' ? IsNull() : Not(IsNull()),
...
const getColumnsFilter = (paging: IPagingRequestQuery) => {
  ...
  Object.keys(paging)
    ...
    .forEach(name => {
      const valueString = Array.isArray(paging[name])
        ? paging[name].join(',')
        : String(paging[name]);
...
export const formatTypeORMPaging = (
  paging: IPagingRequestQuery,
): IPagingTypeORM => {
...
export function getPagingHandler(_?: Class) {
  return async (req: Request, _: Response, next: NextFunction) => {
    if (req.query.order)
      req.query.order = req.query.order.toString().toLocaleUpperCase();
